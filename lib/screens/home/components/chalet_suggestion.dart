...
class Chalet_Suggetion extends StatefulWidget {
  const Chalet_Suggetion({super.key});

  @override
  State<Chalet_Suggetion> createState() => Chalet_SuggetionState();
}

class Chalet_SuggetionState extends State<Chalet_Suggetion> {
  bool didDispose = false;

  late List<Chalet_Suggest>? chaletsugge = [];
  // var allcompany;

  Future<void> _getallsuggetchalet() async {
    // allcompany = await ApiService.fetchServiceDetails(1);
    chaletsugge = await ApiCompany.fetchCompanylastChalet();
    didDispose = true;
    setState(() {});
    //print(companyhight!.map((e) => e.name));
    // print(allcompany['name']);
    // print(chaletsugge!.map((e) => e.name));
  }
                                ...
                                InkWell(
                                  child: Container(
                                    ...
                                    child: CachedNetworkImage(
                                      imageUrl: chaletsug.Data.toString(),
                                      fit: BoxFit.cover,
                                      height: double.infinity,
                                    ),
                                    // Image.network(
                                    //   "${chaletsug.Data.toString()}",
                                    //   fit: BoxFit.cover,
                                    //   height: double.infinity,
                                    // ),
                                  ),
